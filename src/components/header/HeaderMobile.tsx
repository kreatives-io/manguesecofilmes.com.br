...
export default function HeaderMobile({ links }: NavProps) {
    ...
    const menuAnimationClass = useMemo(
      () => `${isToggleOpen ? "h-screen" : "h-10"} min-h-24 transition-all ease-in-out duration-300`,
      [isToggleOpen]
    );
  
    const opacityAnimation = useMemo(
      () => `${isToggleOpen ? "opacity-1" : "opacity-0"} transition-all ease-in-out duration-300`,
      [isToggleOpen]
    );
  
    return (
      <div className={`flex ${menuAnimationClass} w-full lg:hidden justify-center align-center self-center relative container`}>
        <div className="flex w-full px-4">
          ...
          <div className="absolute top-2 right-0 px-4">
            {isToggleOpen ? (
              ...
              <CiMenuBurger onClick={toggleNavBar} size={30} className="text-persian" />
            )}
          </div>
        </div>
      </div>
